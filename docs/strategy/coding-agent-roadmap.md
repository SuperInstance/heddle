...
- the local control plane now has a workstation-style browser shell instead of only a read-only dashboard: sessions render as sidebar + conversation + review inspector, heartbeat renders as task list + detail + run history, and the web client is owned by `src/web-v2`
...
- finish wiring the browser workstation shell into real `send message` and `continue` session mutations so the web control plane can drive chat sessions directly instead of primarily inspecting them
